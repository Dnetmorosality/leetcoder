...
class Solution {
    public List<List<Integer>> fourSum(int[] nums, int target) {
        ...
        while (left + 2 < right) {
            if ((long)nums[left] + nums[left + 1] + nums[left + 2] + nums[right] > target) {
                right--;
        ...
        if (terms.stream().allMatch(item -> Objects.equals(item, terms.getFirst()))) {
            if ((long) terms.getFirst() * 4 == target) {
            ...
            for (int j = i + 1; j < terms.size() - 2; j++) {
                for (int k = j + 1; k < terms.size() - 1; k++) {
                    for (int l = k + 1; l < terms.size(); l++) {
                        if ((long)terms.get(i) + terms.get(j) + terms.get(k) + terms.get(l) == target) {
